...
if mode == "Portfolio (Equity + Bonds) (Variance-Covariance)":
    ...
    if st.button("Run Portfolio VaR"):
        ...
        # Correlation matrix
        fig_corr = plot_correlation_matrix(results[asset_names])  # log returns
        st.pyplot(fig_corr)

        # Individual return histograms
        fig_hists = plot_individual_distributions(results[asset_names]) 
        st.pyplot(fig_hists)

        # Portfolio PnL vs VaR
        fig_pnl = plot_portfolio_pnl_vs_var(results[['PnL', 'VaR_Breach']], results['var_portfolio'], confidence)
        st.pyplot(fig_pnl)
